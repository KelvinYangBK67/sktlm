...
from sktlm.corpus.dataset import load_canonical_segments
...
def test_brhajj_record_prefix_does_not_consume_body() -> None:
    source = (
        "BJ_01.01a/.mūrtitve parikalpitaḥ śaśabhṛto "
        "vartmāpy aśeṣaṃ jagat//(śā.vi)"
    )
    result = normalize_canonical_iast(source).text
    assert result.startswith("mūrtitve parikalpitaḥ")
    assert "aśeṣaṃ jagat||" in result
    assert "BJ_01.01a" not in result
