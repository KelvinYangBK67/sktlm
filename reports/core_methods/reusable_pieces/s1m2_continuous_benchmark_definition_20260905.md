...
### Factorized representative audit attempt 03: numerical-stability gate

A separately prepared attempt 03 started from the earlier nonfinite-only SHA
`2fea66aba9dc569759ac786c11eecf40c6ad654d`, before the schema-typed TSV
checkpoint. Its distinct task/run identity is attempt 03, although its copied
state payload incorrectly retains `attempt: 2`; the failed directory is
preserved without alteration. The job ran no training or inference.

After completing the IAST keyed TSV phase, the audit stopped at JSONL record
16. Piece segmentation entropy was `0.007619114682708593` in the optimization-7
reference and `0.007619114680466055` in the factorized result. The `2.24e-12`
absolute / approximately `2.94e-10` relative difference narrowly exceeds the
frozen `rtol=1e-10`, `atol=1e-12` contract. Attempt-03 state and stderr SHA-256
values are `0049426d04a0a00b6398064b07d64c49fb9d4a4c56204e703ec12afa2eaa3b95`
and `84f2dda5b10be2dad8c4d417ca022a2f53a76b0d2eec55383f01b59e7ad2d1c2`.

The shared evaluator had computed piece entropy as the difference between two
large token-level aggregates. The legacy route instead accumulates each form's
conditional entropy before outer posterior weighting, which is numerically
stable when total piece ambiguity is small. The corrected shared prefix DP now
carries conditional expected raw score as a forward expectation-semiring
quantity. Each endpoint obtains its own conditional entropy, and outer
aggregation follows the legacy decomposition. This remains an exact
factorization: candidate support, path weights, partition, piece marginals,
lexical marginals, rules, boundaries, and top-K support are unchanged; no
per-form lattice is created. Focused pieces and latent suites pass (`40` and
`54` tests).

A fixed cheap paired probe and seven-artifact comparison against optimization
12 are required before another large read-only audit is justified.
