...
from sktlm.corpus.splits import DEFAULT_SPLIT_SEED, assign_split, make_document_id
...
BRHAJJ_RECORD_PREFIX_RE = re.compile(r"^\s*BJ_\d{2}\.\d{2}[ab]?/\.")
    ...
    for original_line in text.split("\n"):
        ...
        if DECORATION_ONLY_RE.match(line):
            ...
            if decorated and any(character.isalnum() for character in decorated.group("body")):
                ...
                replacements += 2
        # Brhajjataka records use BJ_01.01a/. as an exact line locator. It
        # must be removed before the generic siglum rule, which would
        # otherwise consume the following Sanskrit body up to its first digit.
        line, count = BRHAJJ_RECORD_PREFIX_RE.subn("", line, count=1)
        replacements += count
        line, count = _strip_structural_prefixes(line)
        replacements += count
...
def _write_summary(
    ...
    *,
    ...
    lines = [
        ...
        "selection_policy: exact paths from configs/corpus/gretil_whitelist.txt",
        "canonical_script: iast",
...
def _common_parser(description: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--whitelist",
        type=Path,
        default=Path("configs/corpus/gretil_whitelist.txt"),
    )
    parser.add_argument(
        "--output-root",
        "--canonical-root",
        dest="canonical_root",
        type=Path,
        default=Path("data/intermediate/gretil/formal_extracted_gretil_iast"),
    )
    parser.add_argument(
        "--manifest",
        type=Path,
        default=Path("data/manifests/gretil_extraction_manifest.csv"),
    )
...
def main(argv: list[str] | None = None) -> None:
    parser = _common_parser("Build the formal whitelist-only GRETIL extraction candidate")
    parser.add_argument(
        "--report-dir",
        type=Path,
        default=Path("reports/cleaning/generated/formal_extraction"),
    )
...
def validate_main(argv: list[str] | None = None) -> None:
    parser = _common_parser("Validate the formal whitelist-only GRETIL extraction candidate")
    parser.add_argument("--split-seed", default=DEFAULT_SPLIT_SEED)
    args = parser.parse_args(argv)
