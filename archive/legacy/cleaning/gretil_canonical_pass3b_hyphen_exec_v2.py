"""Pass 3B v2: execute and refine verified hyphen normalization.

This is a complete rebuild from Pass 3A, not a patch applied on top of the
Pass 3B v1 output.

Rules
-----

1. GENERAL_SEPARATOR_TO_SPACE
   In an explicitly whitelisted file, any hyphen classified as SEPARATOR by
   the dry-run classifier is replaced with one ASCII space.

2. REDUNDANT_HYPHEN_BEFORE_SPACE_REMOVED
   In confirmed prose/text families (excluding files reserved for apparatus
   cleanup), a review-form ``lexical-<space>lexical`` has only the hyphen
   removed. The existing whitespace is preserved.

       sva-yoga- māyā-balaṃ
       -> sva yoga māyā balaṃ

   This rule is deliberately NOT enabled for:
       6_sastra/7_ayur/vagaah_u.txt
       4_rellit/vaisn/vaimp__u.txt

   Those files contain editorial/apparatus material that will be handled by
   later source-specific cleanup.

3. APOSTROPHE_HYPHEN_REMOVED
   In any whitelisted file, ``' - lexical`` without intervening whitespace is
   normalized by deleting the hyphen:
...
REVIEW and NON_SEPARATOR occurrences that do not match the two explicit
refinement rules remain untouched.
...
Output::
    ...
    data/canonical_candidate/pass3b_v2_hyphen_normalized_gretil_iast

Reports::

    data/_reports/pass3b_hyphen_normalization_v2/
...
IMPLEMENTATION = "hyphen-executor-v2"
...
SUMMARY_FILENAME = "gretil_pass3b_v2_hyphen_summary.txt"
FILE_AUDIT_FILENAME = "gretil_pass3b_v2_hyphen_file_audit.csv"
AUTO_FILENAME = "gretil_pass3b_v2_hyphen_auto.csv"
REVIEW_FILENAME = "gretil_pass3b_v2_hyphen_review.csv"
NONSEPARATOR_FILENAME = "gretil_pass3b_v2_hyphen_nonseparator.csv"
...
# These files remain eligible for GENERAL_SEPARATOR_TO_SPACE and
# APOSTROPHE_HYPHEN_REMOVED, but are excluded from the more permissive
# redundant "- " cleanup because their residual hyphens mix with apparatus.
REDUNDANT_SPACE_EXCLUSIONS = frozenset(
    {
        "4_rellit/vaisn/vaimp__u.txt",
        "6_sastra/7_ayur/vagaah_u.txt",
    }
...
REDUNDANT_HYPHEN_SPACE_FILES = frozenset(
    HYPHEN_BOUNDARY_FILES
    - REDUNDANT_SPACE_EXCLUSIONS
)
...
def _matches_redundant_hyphen_before_space(
    ...
    *,
    ...
    """Recognize conservative ``lexical- whitespace lexical`` cleanup."""

    if path not in REDUNDANT_HYPHEN_SPACE_FILES:
        return False
    ...
    if (
        left is None
        or not _is_lowercase_lexical(left)
    ):
        return False
    ...
    next_index, next_character = next_item
    ...
    # Do not cross into a protected span after whitespace.
    if _inside_protected(
        next_index,
        protected,
    ):
...
def _matches_apostrophe_hyphen_lexical(
    ...
    *,
    ...
    """Recognize ``'-x`` and delete only the hyphen."""
...
def _classify_replacement(
    ...
    *,
    ...
    """Return replacement plus original classifier decision/reason."""
    ...
    if classification.decision == "SEPARATOR":
        return (
            Replacement(
                index=index,
                replacement=" ",
    ...
    # Refinements apply only to REVIEW-like residual forms. Explicit
    # NON_SEPARATOR cases stay protected.
...
def _validate_roots(
    ...
    if source == destination:
        raise ValueError(
            "Pass 3B v2 output root must not equal input root"
        )
    ...
    try:
        ...
        raise ValueError(
            "Pass 3B v2 output root must not be inside input root"
        )
    ...
    *,
    ...
    if not input_root.is_dir():
        raise FileNotFoundError(
            f"Pass 3B v2 input root does not exist: {input_root}"
        )
...
def _write_summary(
    ...
    *,
    ...
    lines = [
        "Formal GRETIL Pass 3B v2 hyphen normalization",
        "=============================================",
        (
            "implementation: "
            f"{IMPLEMENTATION}"
        ...
        (
            "  general_separator_to_space: "
            f"{general_separator_replacements}"
        ),
        (
            "  redundant_hyphen_before_space_removed: "
            f"{redundant_space_replacements}"
        ),
        (
            "  apostrophe_hyphen_removed: "
            f"{apostrophe_replacements}"
        ),
        (
            "  total_auto: "
            f"{total_auto}"
        ...
        *[
        ...
        (
            "  lexical-<space>lexical review forms "
            "drop only the redundant hyphen"
        ),
        (
            "  apostrophe-hyphen-lexical review forms "
            "drop only the hyphen"
        ),
        (
            "  vaimp and vagaah are excluded from "
            "redundant-space cleanup pending apparatus review"
        ),
        (
            "  other REVIEW and NON_SEPARATOR cases "
            "remain unchanged"
        ),
        "",
...
def main(
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Execute verified hyphen normalization "
            "with conservative REVIEW refinements"
        )
    )
