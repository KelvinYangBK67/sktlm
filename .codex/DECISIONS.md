...
109. S1M2 pre-VM Round 1 is a new, scoped VM measurement contract and does not reopen or reinterpret the completed S1M1 cloud-medium decision. It runs the same frozen Devanagari-continuous representative workload sequentially at 4, 8, 12, 16, 20, and 24 workers. A configuration must pass scientific, provenance, artifact, memory, and storage gates. A fastest configuration with at least a 10% advantage over the runner-up wins directly; otherwise configurations within the 10% practical-tie envelope are ordered by lower process-tree peak RSS, watched peak storage, total CPU, canonical-reducer stall, and worker count.
