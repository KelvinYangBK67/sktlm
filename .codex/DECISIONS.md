...
105. Bounded Codex work on Windows uses the generic tracked framework in `.codex/automation/`; task-specific mutable config/state, frozen prompt copies, exact thread identity, and logs belong only under ignored `artifacts/codex_automation/<automation_id>/`. Scheduled recurrence stays anchored at `Start + n * IntervalMinutes`; manual wakes never rebuild or re-anchor the trigger. New threads are permitted only when stored `thread_id` is null, every continuation resumes and verifies the exact stored ID, and `resume --last` is forbidden. `WAITING_EXTERNAL`, `COMPLETE`, repository divergence, invalid markers, and launcher failures fail closed by disabling the task. Installing or starting a concrete bounded task remains an explicit researcher action.
