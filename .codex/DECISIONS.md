...
80. Formal four-cell/two-N/A aggregation is validated. S1M1 still cannot become `READY_TO_FREEZE`, and no completed-cell large artifact can be marked safe to delete, until exact training-final scorer/reuse state is compactly exported, SQLite/WAL and compact hashes are recorded, and read-back consistency passes.
...
83. S1M1 scientific analysis may close before archival completeness. Its current state is `SCIENTIFIC_ANALYSIS_COMPLETE`, `ARCHIVAL_COMPLETENESS_PENDING`, and `NOT_FROZEN`; the sole archival blocker is completed-cell training-final SQLite/WAL compact identity plus read-back evidence.
