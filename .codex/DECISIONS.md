...
70. Formal S1M1 aggregation and scientific analysis are complete. Archival completeness, source-deletion decisions, and milestone freeze remain deferred until the compact training-final state gate passes.
...
80. Formal four-cell/two-N/A aggregation is validated. Freeze remains pending on the researcher-selected selective archival/deletion-readiness/final-audit sequence, not because every completed-cell SQLite database is a scientific prerequisite.
...
83. S1M1 scientific analysis may close before archival completeness. Its current state is `SCIENTIFIC_ANALYSIS_COMPLETE`, `ARCHIVAL_COMPLETENESS_PENDING`, and `NOT_FROZEN`; SQLite-derived association state is supplementary microscopic evidence rather than an input required to validate the formal representation analysis.
84. `notes/**` is strictly local-only. Codex must never read, inspect, search, modify, track, stage, commit, restore, delete, move, copy, or force-add anything under `notes/`.
...
87. Repository tooling and Codex may classify scientific artifacts as `RETAIN`, `SAFE_TO_DELETE`, `NOT_READY`, or `NOT_SAFE`, but must never delete them. Every deletion requires separate manual researcher authority.
