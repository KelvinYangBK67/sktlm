...
103. S1M2 continuous workload selection is frozen before timing: take the top two documents by maximum punctuation-free span (then squared-span proxy and phonemes), remove them, and take nearest distinct 25th/50th/75th percentile documents ranked by squared-span proxy, maximum span, phonemes, and deterministic path tie break. Selection uses matched M0-prime IAST/M0 Devanagari frontend structure only, never model outcome or benchmark runtime.
104. S1M2 immutable candidate/form support and shared piece-prefix transition topology may be compiled once during pass 1 into validated, reconstructible, document-local bounded archives and reused by later passes and final inspection. Piece parameters/scores, segmentation-prior weights, forward/backward state, posteriors, expected counts, and top-K score state remain mutable and must be recomputed exactly every phase. Opt16 is accepted on exact changed-parameter, resume, serial/parallel, and canonical-artifact equivalence; after Opt15/Opt16, local optimization stops unless new profiling evidence identifies another dominant structural mechanism.
105. Bounded Codex work on Windows uses the generic tracked framework in `.codex/automation/`; task-specific mutable config/state, frozen prompt copies, exact thread identity, and logs belong only under ignored `artifacts/codex_automation/<automation_id>/`. Scheduled recurrence stays anchored at `Start + n * IntervalMinutes`; manual wakes never rebuild or re-anchor the trigger. New and exact-resume invocations use `--approve-for-me` alone because it supplies the workspace-write sandbox; an additional `--sandbox` is forbidden, as are the dangerous bypass and `resume --last`. Exact TaskName/runtime-root collisions fail closed, but unrelated historical `SKTLM-*` one-shot tasks and idle generic peers may coexist. Per-task and canonical-repository mutexes prevent same-task re-entry and concurrent same-repository Codex execution; repository-lock contention skips the wake without failing healthy state. `WAITING_EXTERNAL`, `COMPLETE`, repository divergence, invalid markers, and launcher failures fail closed by disabling the task. Only a proven pre-thread `LAUNCHER_ERROR` with empty thread identity, intact prompts, normal repository gates, and exact disabled task/config/trigger identity may use `RecoverPreThread`; it preserves the schedule and never injects a thread ID. Installing, recovering, or starting a concrete bounded task remains an explicit researcher action.
106. After `thread.started`, exact-thread continuation is durable across quota, CLI, transport, and nonzero-exit interruption. The runner atomically persists the observed ID while Codex is still running and salvages it again before failure handling. NEW threads still require a clean local/remote-equal checkout; established threads instead require an exact deterministic checkpoint of unstaged diff, staged diff, non-ignored untracked content, local HEAD, and remote HEAD. An unchanged dirty or local-ahead checkpoint may resume; any between-wake external change fails closed. Post-thread temporary failure becomes `INTERRUPTED_RECOVERABLE` without disabling the task. `WAITING_DETACHED` is distinct from researcher-blocking `WAITING_EXTERNAL`: it retains an immutable artifacts-local job manifest with job/command/process/PID/start/result/exit identity, polls without invoking Codex while the identity runs, and exact-resumes the same thread once on success or failure. Legacy state may use researcher-only `RecoverInterrupted -AdoptWorkspace` to salvage its unique JSONL thread ID and adopt the present workspace checkpoint without changing files or the fixed trigger.
107. The Opt16 topology archive is a reconstructible engineering cache, never authoritative scientific state. Missing archives and narrowly classified format, identity, truncation, decompression, or structural-validation failures are repaired by deterministic regeneration from the frozen input and fixed structural configuration followed by atomic replacement. Mutable scores, posteriors, forward/backward state, and expected counts are never restored from the archive; unexpected programming and inference exceptions fail loudly.
108. The frozen S1M2 production universe has exactly six cells: M0 IAST `surface_word`, M0 IAST `legacy_joined`, M0-prime IAST `continuous`, M0 Devanagari `surface_word`, M0 Devanagari `legacy_joined`, and M0 Devanagari `continuous`. Original M0 IAST `continuous` remains scientifically excluded and the production contract fails closed rather than substituting it. Worker count is engineering-only and cannot alter a cell's scientific configuration.
109. S1M2 pre-VM Round 1 is a scoped engineering worker calibration and does not reopen or reinterpret the completed S1M1 cloud-medium decision or the frozen representative/stress workloads. It uses 72 distinct non-stress documents selected deterministically from the frozen M0 Devanagari-continuous static-pressure ordering by lower-median sampling of 72 contiguous strata, with three passes and at most 256 lines per document. The same calibration workload runs concurrently on six equivalent core hosts at 4, 8, 12, 16, 20, and 24 workers, mapped in order to `core-01` through `core-06`. A configuration must pass scientific, provenance, artifact, memory, and storage gates. A fastest configuration with at least a 10% advantage over the runner-up wins directly; otherwise configurations within the 10% practical-tie envelope are ordered by lower process-tree peak RSS, watched peak storage, total CPU, canonical-reducer stall, and worker count.
110. S1M2 Round 2 consumes Round 1's machine-readable `WINNER_WORKERS` and contains exactly the frozen six-job readiness matrix: Devanagari continuous stress; M0-prime IAST continuous representative; Devanagari continuous representative; and smoke runs for Devanagari `surface_word`, Devanagari `legacy_joined`, and IAST `surface_word`. Round 2 gates are engineering readiness evidence only; failing them does not authorize Opt17, approximate inference, candidate pruning/limit tuning, grammar changes, or any scoring/posterior change. A final six-cell full-M0 launch plan may be generated only from a PASS Round 2 result, and plan generation never starts production.
111. Generic cloud operations take experiment identity from a strict tracked contract rather than a bridge-coded branch or workload. The contract owns branch, deployment transport, input sets, remote roots, host assignment, collection profiles, audit identity, and completion identity. Production code transport is explicitly `git_remote` or `git_bundle`; a bundle deployment is clean/published/exact-SHA, locally and remotely verified, and fast-forward-only.
112. S1M2 Round 1 and Round 2 are six-host parallel control-plane stages, justified by the accepted S1M1 core-host equivalence evidence. Round 1 maps worker counts 4, 8, 12, 16, 20, and 24 respectively to `core-01` through `core-06` for the same frozen M0 Devanagari-continuous worker-calibration workload. Round 2 maps its six frozen workloads respectively to `core-01` through `core-06`, all at the machine-readable Round 1 winner. A single shared `s1m2-vm-01` role and sequential launch are obsolete and forbidden.
113. S1M2 execution bundling may divide a document only between existing complete ObservedSegment values emitted by the unchanged trainer frontend. Bundle completion order is execution-only: scientific counts, pass metrics, topology records, document transactions, and checkpoints remain reduced in canonical document/segment order. Completed futures must leave true inflight capacity immediately; ready-but-blocked results must not prevent refill. The bundle plan is validated and resume-bound execution provenance, not learned scientific state.
114. **Supersedes Decisions 110 and the Round-2 portion of 112.** The former six-workload Round 2 readiness stage is retired because its expected information gain no longer justifies a mandatory pre-full gate after execution bundling. Active Round 2 recalibrates worker count using the frozen M0 Devanagari-continuous representative and stress workloads at workers 12, 16, and 24, with workload-specific plans fixed at target pressure 279047 and complete ObservedSegment atomicity. A candidate is eligible only after both workloads pass engineering safety gates. A resolved Round 2 winner directly supplies the engineering worker count for the unchanged full six-cell production plan. Worker 20 remains an interpolation candidate only when the primary matrix is decision-critical; no replacement readiness stage exists.
115. S1M2 production executes each training pass and final inspection in a separate monitored process while retaining one scientific run, one `learner.sqlite`, one configuration signature, and one canonical checkpoint history. `--next-pass-only` is execution-only: it completes exactly the active or next unfinished pass and exits after the transactional final checkpoint. Production chooses every next phase from the authoritative SQLite checkpoint, records per-phase commands/timestamps/return codes/metrics, and fails closed before launching a later phase after any nonzero exit.
116. S1M2 execution-bundle plan schema v2 records the deterministic UTF-8 byte offset of each bundle's first source line so workers seek directly to their range. This offset and its plan digest are reconstructible execution identity, not scientific training identity; v1 plans fail closed and must be rematerialized. Round 4 may remove redundant I/O, traversal, transient objects, and immutable arithmetic only while preserving exact scientific semantics, canonical/floating-point accumulation order, and all existing RAM-related bounds.
117. Generic cloud-host bootstrap is an idempotent, receipt-backed WSL control-plane operation over one arbitrary `BridgeConfig` host profile. It may initialize only an explicitly supplied blank data device or resume a recognizable tool-owned intermediate; existing valid mounts are reused and all root/system, conflicting, partitioned, signed, or unexpectedly mounted devices fail closed. Code deployment is the clean published current branch at exact HEAD through the existing verified Git-bundle path; generic bootstrap transfers only frozen canonical and representation inputs and stops at `READY` without launching any scientific workload.
118. Generic bootstrap derives release identity from the clean attached local Git branch and exact HEAD, independently of optional `BridgeConfig.branch`, and requires the corresponding published remote branch HEAD to match exactly. Recovery may remove and rebuild only the exact tool-owned `/opt/python-3.11.9` prefix or configured `venv-py311` directory when its partial layout is recognizable; symlinks, non-directories, unexpected contents, and a venv bound to another base interpreter fail closed.
