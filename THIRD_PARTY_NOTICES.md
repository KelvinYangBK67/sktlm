# Third-party notices

- **GRETIL** — The Göttingen Register of Electronic Texts in Indian Languages
  supplied source texts used by the corpus pipeline. GRETIL and source-edition
  attribution must be preserved. See [`DATA_LICENSE.md`](DATA_LICENSE.md).
- **SentencePiece** — Copyright Google Inc.; distributed under the Apache
  License 2.0.
- **PyTorch** — Copyright PyTorch contributors; distributed under the BSD
  3-Clause License.
- **PyYAML** — Copyright Kirill Simonov and contributors; distributed under
  the MIT License.
- **regex** — Derived in part from CPython's `re` module under the CNRI Python
  1.6 license; additions and alterations are distributed under the Apache
  License 2.0.

These notices are informational and do not replace the license texts supplied
by the respective upstream projects.
